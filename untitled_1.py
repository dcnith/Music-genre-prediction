...
tsne = TSNE(n_components=3, random_state=0, method='exact')
...
X_sm, y_sm = smenn.fit_resample(X_new, y1)
...
scores=cross_val_score(classifier,X_sm,y_sm,cv=skfold)

scores
print(np.mean(scores))
print("Accuracy: %.3f%% (%.3f%%)" % (scores.mean()*100.0, scores.std()*100.0))

y_pred = cross_val_predict(classifier,X_sm,y_sm,cv=skfold)
...
scores=cross_val_score(xgb_model,X_sm,y_sm,cv=skfold)
scores
print(np.mean(scores))
print("Accuracy: %.3f%% (%.3f%%)" % (scores.mean()*100.0, scores.std()*100.0))

y_pred = cross_val_predict(xgb_model, X_sm,y_sm, cv=skfold)
...
data_test.drop(['song_id', 'album','artist','lyrics','track','release_date'], axis=1, inplace=True)
...
X_new_test=pca.fit_transform(data_test)
X_new

##### For Test data transformation

tsne_test = tsne.transform(data_test)
